...
if [ $DVB_NETWORK_FLAG = yes ] ; then
	find $GXSRC_PATH/app/youtube -iname "*.c" |xargs grep SIGNAL_HANDLER |awk '{print  $3}'|awk -F'(' '{print "extern int "$1 "(const char* widgetname, void *usrdata);"}' >>./$file_name
fi
if [ $DVB_ECOS_WIFI = yes ] ; then
	find $GXSRC_PATH/app/netapps -iname "*.c" |xargs grep SIGNAL_HANDLER |awk '{print  $3}'|awk -F'(' '{print "extern int "$1 "(const char* widgetname, void *usrdata);"}' >>./$file_name
fi
...
if [ $DVB_NETWORK_FLAG = yes ] ; then
	find $GXSRC_PATH/app/youtube -iname "*.c" |xargs grep SIGNAL_HANDLER |awk '{print  $3}'|awk -F'(' '{print "\tret |= SIGNAL_CONNECT("$1");"}' >>./$file_name
fi

if [ $DVB_ECOS_WIFI = yes ] ; then
	find $GXSRC_PATH/app/netapps -iname "*.c" |xargs grep SIGNAL_HANDLER |awk '{print  $3}'|awk -F'(' '{print "\tret |= SIGNAL_CONNECT("$1");"}' >>./$file_name
fi
