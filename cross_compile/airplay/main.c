...
#define  KEY_CNT_MAX 64

const char  server_name[128] = DEFAULT_NAME;
static dnssd_t *dnssd = NULL;
static raop_t *raop = NULL;
static logger_t *render_logger = NULL;
static bool audio_sync = false;
static int videosink= 1;
...
static bool use_video = 0;
static unsigned char compression_type = 0;
static int audiosink= 0;
static int  audiodelay = -1;
static bool use_audio = 0;
static bool new_window_closing_behavior = true;
static bool close_window;
static char video_parser[32] = "h264parse";//h264parse
...
static char video_dumpfile_name[32] = "videodump";
...
static char coverart_filename[128] = "art_name";
static char pairing_register[64] = ".uxplay.register";
static char registered_keys[KEY_CNT_MAX][64]= {{0},};
static int registered_keys_cnt=0;


static char keyfile[64] = "airport.key";
static char dacpfile[64] = ".uxplay.dacp";
static unsigned char mac_address[64] = {0};

typedef struct airplay_options{
	char name[256];
	char password[64];
	unsigned short port;
	char hwaddr[6];
...
airplay_options_t options={0};


/* logging */

static void log_com(int level, const char* format, ...) {
    va_list vargs;
...
#define LOGD(...) log_com(LOGGER_DEBUG, __VA_ARGS__)
#define LOGI(...) log_com(LOGGER_INFO, __VA_ARGS__)
#define LOGW(...) log_com(LOGGER_WARNING, __VA_ARGS__)
#define LOGE(...) log_com(LOGGER_ERR, __VA_ARGS__)
...
static void dump_audio_to_file(unsigned char *data, int datalen, unsigned char type) {
    ...
    if (!audio_dumpfile && type != previous_audio_type) {
        char suffix[20];
        previous_audio_type = type;
        ...
        strcat(audio_dumpfile_name,suffix);
        audio_dumpfile = fopen(audio_dumpfile_name,"w");
        if (audio_dumpfile == NULL) {
            LOGE("could not open file %s for dumping audio frames",audio_dumpfile_name);
        }
    }
...
static void dump_video_to_file(unsigned char *data, int datalen) {
    ...
    if (!video_dumpfile) {
        if (video_dump_limit) {
            ...
            strcat(video_dumpfile_name,suffix);
	}
        strcat(video_dumpfile_name,".h264");
        video_dumpfile = fopen (video_dumpfile_name,"w");
        if (video_dumpfile == NULL) {
            LOGE("could not open file %s for dumping h264 frames",video_dumpfile_name);
        }
    }
...
static int parse_hw_addr(const char *str, char *hwaddr, int hwaddrlen)
{
	int slen, i;
...
static bool validate_mac(char * address) {
    char c;
...
static bool option_has_value(const int i, const int argc, char* option, const char *next_arg) {
    if (i >= argc - 1 || next_arg[0] == '-') {
        LOGE("invalid: \"%s\" had no argument", option);
        return 1;
     }
    return true;
}
...
static void parse_arguments (int argc, char *argv[]) {
	...
	if (arg == "-allow") {
	    ...
	    if (i < argc - 1 && *argv[i+1] != '-') {
            ...
            if (!option_has_value(i, argc, arg, argv[i+1])) exit(1);
            strncpy(video_parser, "h264parse",sizeof("h264parse"));//h264parse
        } else if (arg == "-vd") {
            if (!option_has_value(i, argc, arg, argv[i+1])) exit(1);
            //video_decoder.erase();
            ...
            memset(video_parser,0x0,sizeof(video_parser));
            strncpy(video_parser, "h264parse",sizeof("h264parse"));//h264parse
            //video_decoder.erase();
            ...
            if (i < argc - 1 && *argv[i+1] != '-') {
                ...
                if (get_value (argv[++i], &n)) {
                    ...
                    if (option_has_value(i, argc, arg, argv[i+1])) {
                         memset(video_dumpfile_name,0x0,sizeof(video_dumpfile_name));
                        strcat(video_dumpfile_name,argv[++i]);
                    }
                } else {
                    memset(video_dumpfile_name,0x0,sizeof(video_dumpfile_name));
                    strcat(video_dumpfile_name,argv[i]);
                }
                if (!file_has_write_access(video_dumpfile_name)) {
            ...
            if (i < argc - 1 && *argv[i+1] != '-') {
                ...
                if (get_value (argv[++i], &n)) {
                    ...
                    audio_dump_limit = n;
                    if (option_has_value(i, argc, arg, argv[i+1])) {
                        memset(audio_dumpfile_name,0x0,sizeof(audio_dumpfile_name));
                        strcat(audio_dumpfile_name,argv[++i]);
                    }
                } else {
                        memset(audio_dumpfile_name,0x0,sizeof(audio_dumpfile_name));
                        strcat(audio_dumpfile_name,argv[i]);
                }
                if (!file_has_write_access(audio_dumpfile_name)) {
            ...
            if (option_has_value(i, argc, arg, argv[i+1])) {
                ...
                memset(coverart_filename,0x0,sizeof(coverart_filename));
                strcat(coverart_filename,argv[++i]);
                if (!file_has_write_access(coverart_filename)) {
	    ...
	    if (i < argc - 1 && *argv[i+1] != '-') {
                ...
                if (!file_has_write_access(pairing_register)) {
...
static int register_dnssd() {
    ...
    if ((dnssd_error = dnssd_register_raop(dnssd, raop_port))) { //audio
    ...
    if ((dnssd_error = dnssd_register_airplay(dnssd, airplay_port))) {//video
...
static int start_dnssd(const char* hw_addr, int hw_addr_len, const char* name, int name_len) {
    int dnssd_error;
    ...
    dnssd = dnssd_init(name, name_len, hw_addr, hw_addr_len, &dnssd_error, require_pw);
    if (dnssd_error) {
    ...
    printf("name %s \n",name);
    printf("hw_addr 0x%x %x %x %x %x %x \n",hw_addr[0],hw_addr[1],hw_addr[2],hw_addr[3],hw_addr[4],hw_addr[5]);
    //printf("hw_addr 0x%x %x %x %x %x %x \n",dnssd->hw_addr+0,dnssd->hw_addr+1,dnssd->hw_addr+2,dnssd->hw_addr+3,dnssd->hw_addr+4,dnssd->hw_addr+5);
...
// Server callbacks
 ...
 void export_dacp(void *cls, const char *active_remote, const char *dacp_id) {
      if (strlen(dacpfile)) {
        FILE *fp = fopen(dacpfile, "w");
        if (fp) {
            fprintf(fp,"%s\n%s\n", dacp_id, active_remote);
            fclose(fp);
        } else {
            LOGE("failed to open DACP export file \"%s\"", dacpfile);
        }
    }
}
...
void conn_teardown(void *cls, bool *teardown_96, bool *teardown_110) {
 ...
 void audio_get_format (void *cls, unsigned char *ct, unsigned short *spf, bool *usingScreen, bool *isMedia, uint64_t *audioFormat) {
    ...
    if (strlen(coverart_filename)) {
        write_coverart(coverart_filename, (const void *) empty_image, sizeof(empty_image));
    }
}
...
void audio_set_coverart(void *cls, const void *buffer, int buflen) {
    if (buffer && strlen(coverart_filename)) {
        write_coverart(coverart_filename, buffer, buflen);
        LOGI("coverart size %d written to %s", buflen,  coverart_filename);
    }
}
...
void register_client(void *cls, const char *device_id, const char *client_pk, const char *client_name) {
    ...
    memcpy(registered_keys[registered_keys_cnt],client_pk,strlen(client_pk));
    registered_keys_cnt++;
    registered_keys_cnt = registered_keys_cnt%KEY_CNT_MAX;
    if (strlen(pairing_register)) {
        FILE *fp = fopen(pairing_register, "a");
        if (fp) {
 ...
 bool check_register(void *cls, const char *client_pk) {
    if (!registration_list) {
        /* we are not maintaining a list of registered clients */
        return true;
    }
    ...
    int i=0;
    for(i=0 ;KEY_CNT_MAX ;i++){
        if (memcmp(registered_keys[i], client_pk, 64)==0) {
...
static int start_raop_server (unsigned short *display, unsigned short *tcp, unsigned short *udp, bool debug_log) {
    ...
    raop = raop_init(max_connections, &raop_cbs, options.hwaddr, keyfile);
    if (raop == NULL) {
...
//init param
    ...
    udp[0] = 7011; udp[1] = 6001; udp[2] = 6000;
    display[0] = 800;//width
    display[1] = 600;//height
    display[2] = 60;//refreshRate
    display[3] = 30;//maxFPS
    display[4] = 0;//overscanned
    use_audio = 0;
    use_video = 0;
    dump_audio = 0;
    dump_video = 1;
    ...
    if (dump_video) {
        if (video_dump_limit > 0) {
             printf("dump video using \"-vdmp %d %s\"\n", video_dump_limit, video_dumpfile_name);
	} else {
    ...
    if (dump_audio) {
        if (audio_dump_limit > 0) {
            printf("dump audio using \"-admp %d %s\"\n", audio_dump_limit, audio_dumpfile_name);
        } else {
    ...
    if (use_audio) {
      audio_renderer_init(render_logger, &audiosink, &audio_sync, &video_sync);
    } else {
        LOGI("audio_disabled");
    }
    ...
    if (use_video) {
        #if 0
        video_renderer_init(render_logger, server_name.c_str(), videoflip, video_parser,
                            video_decoder.c_str(), video_converter.c_str(), &videosink, &fullscreen, &video_sync);
        #endif
    ...
    if (!use_random_hw_addr) {
        if (strlen(mac_address) == 0) {
             char* p_tmp = find_mac();
            memcpy(mac_address, p_tmp, sizeof(mac_address));
            LOGI("using system MAC address %s",mac_address);
        } else {
            LOGI("using user-set MAC address %s",mac_address);
        }
    }
    memcpy(options.name,server_name,sizeof(server_name));
    const char default_hwaddr[] = { 0x48, 0x5d, 0x60, 0x7c, 0xee, 0x22 };
    memcpy(mac_address, default_hwaddr, sizeof(default_hwaddr));
    memcpy(options.hwaddr, default_hwaddr, sizeof(default_hwaddr));
    //validate_mac(mac_address);

    printf("mac_address 0x%x %x  %x  %x  %x  %x \n",mac_address[0],mac_address[1],mac_address[2],mac_address[3],mac_address[4],mac_address[5]);
    parse_hw_addr(mac_address, options.hwaddr,sizeof(options.hwaddr));
    if (0){
        fprintf(stderr,"invalid mac address \"\": address must have form"
                " \"xx:xx:xx:xx:xx:xx\", x = 0-9, A-F or a-f\n");
        exit(1);
    }
    ...
    if (strlen(coverart_filename)) {
        LOGI("any AirPlay audio cover-art will be written to file  %s",coverart_filename);
        write_coverart(coverart_filename, (const void *) empty_image, sizeof(empty_image));
    }
    ...
    printf(" hwaddr len %d  name len %d \n",strlen(options.hwaddr),strlen(options.name));
    if (start_dnssd(options.hwaddr, strlen(options.hwaddr), options.name, strlen(options.name))) {
        goto cleanup;
    }
    ...
    if (relaunch_video || reset_loop) {
        ...
        if (use_video && close_window) {
            ...
            #if 0
            video_renderer_init(render_logger, server_name.c_str(), videoflip, video_parser,
                                video_decoder.c_str(), video_converter.c_str(), &videosink, &fullscreen,
                                &video_sync);
           #endif
            video_renderer_start();
        }
