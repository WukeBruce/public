...
raop_rtp_thread_udp(void *arg)
{
	...
	if (FD_ISSET(raop_rtp->dsock, &rfds)) {
            ...
            //logger_log(raop_rtp->logger, LOGGER_INFO, "raop_rtp_thread_udp type_d 0x%02x, packetlen = %d", type_d, packetlen);
	   ...
	   //printf("raop_rtp->ct =  %d ,ct = 2 (ALAC), ct = 8 (AAC_ELD) (ct = 4 would be AAC-MAIN)\n",raop_rtp->ct);
                     ...
                     unsigned short *data_lport, unsigned char *ct, unsigned int *sr)
{
    ...
    if (raop_rtp->running || !raop_rtp->joined) {
        logger_log(raop_rtp->logger, LOGGER_ERR, "raop_rtp->running %d raop_rtp->joined  %d",raop_rtp->running,raop_rtp->joined);
        MUTEX_UNLOCK(raop_rtp->run_mutex);
        return;
    }
