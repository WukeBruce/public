...
struct partition_table {
	uint32_t              magic;
	uint8_t               count;
	struct partition_info tables[FLASH_PARTITION_V2_NUM];
	uint8_t               write_protect;
	uint8_t               crc32_enable;
	char                  version;
	unsigned int          crc_verify;
} __attribute__ ((packed));
...
int image_merge(const char *conf_file_name, flash *entry, const char *file_name);
int image_split(const char *file_name, const char *dir);

DLLEXPORT flash *flash_load_conf(const char *conf);
...
DLLEXPORT int    flash_partition_dump(flash *entry, const char *partition_name, const char *filename);
