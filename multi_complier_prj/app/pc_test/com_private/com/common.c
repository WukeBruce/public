...
static uint32_t CRC32(uint8_t* pBuffer, uint32_t nSize)
{
...
static void function_test_crc(void)
{
	uint32_t ret  = 0;
	static uint8_t arary_payload[]={
		0x00,0xb0,0x0d,0x00,0x01,0xc1,0x00,0x00,0x00,0x01,0xf0,0x00,
	};//0x2ab104b2

	ret = CRC32(arary_payload,sizeof(arary_payload));

	COM_INFO("crc ret 0x%x\n",ret);
}
...
static int search_array_max_value_index(void)
{
    int a[MAX_NUM];
    ...
    COM_INFO("max=%d,index_max=%d\n\n",a[index_max],index_max);

    return 0;
}


static int search_array_min_value_index(void)
{
    int a[MAX_NUM];
    int i,index_min;    //index_max用于接收最大值下标
    ...
    index_min=0;     //初始化最大值下标

    for(i=1;i<MAX_NUM;++i)
    {
        if(a[i] <= a[index_min])
        {
            index_min=i;     //用下标存储最大值下标，不仅可以找到最大值，也可以找到它的位置
        }
    }

    COM_INFO("min=%d,index_min=%d\n\n",a[index_min],index_min);

    return 0;
}


int search_array_max_min_test(void){
	search_array_max_value_index();
	search_array_min_value_index();

	return 0;
}

static char *media_get_size_str_format2(char *size_str, unsigned long long size)
{
	if (size >= 1024 * 1024 * 1024)
		sprintf(size_str, "%6.2fG Bytes", size * 1.0 / 1024.0 / 1024.0/ 1024.0);
	else if (size >= 1024 * 1024)
		sprintf(size_str, "%6.2fM Bytes", size * 1.0 / 1024.0/ 1024.0);
	else if (size >= 1024)
		sprintf(size_str, "%6.2fK Bytes", size * 1.0 / 1024.0);
	else
		sprintf(size_str, "%6.2fBytes", size * 1.0);

	return size_str;
}

static char *media_get_size_str(char *size_str, unsigned long long size)
{
	return media_get_size_str_format2(size_str,size);
}


int print_size_test(void){
	char size_str1[32]={0};
	memset(size_str1,0x0,sizeof(size_str1));
    int a;

	COM_INFO("input decimal num :\n");
    scanf("%d",&a);
	media_get_size_str(size_str1, (unsigned long long)a);
    COM_INFO("size_str1 %s\n",size_str1);
